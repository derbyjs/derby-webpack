...
function plugin(app, options) {
  ...
  const sourcesRe = new RegExp(
    `(${app.name}|${app.name}_views|vendors|common|runtime).*\.js$`
  );
  ...
  function readManifestAssets(page, filepath) {
    if (!existsSync(filepath)) {
      console.error(
        "No manifest.json file found, and webpack middleware not available. Run pack to build static bundle before starting"
      );
  ...
  app.on("htmlDone", (page) => {
    ...
    const scriptTags = assets
      .map(
        (path) =>
          `<script ${
            scriptCrossOrigin ? "crossorigin " : ""
          }src="${path}" type="text/javascript"></script>`
      )
      .join("\n");
